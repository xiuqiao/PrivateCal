...
-(void)processOp:(int)theOp
{
    switch (theOp) {
        ...
        case 12:    //按下“％”
            if ([self isFirstLaunch]) {
                [self savePassword:self.portraitDisplay.text];
...
- (Boolean)isFirstLaunch{
    ...
    return ![standardUserDefaults boolForKey:@"first_launch"];
}

- (void)saveFirstLaunch{
    NSUserDefaults *standardUserDefaults = [NSUserDefaults standardUserDefaults];
    [standardUserDefaults setBool:YES forKey:@"first_launch"];
    [standardUserDefaults synchronize];
}

- (void)savePassword:(NSString *)password{
    NSUserDefaults *standUD = [NSUserDefaults standardUserDefaults];
    [standUD setValue:password forKey:@"password_key"];
    [standUD synchronize];
}

- (NSString *)getPassword{
    NSUserDefaults *stand = [NSUserDefaults standardUserDefaults];
    return  [stand stringForKey:@"password_key"];
}
